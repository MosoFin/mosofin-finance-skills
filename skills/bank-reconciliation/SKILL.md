...
| 2 | **Connect the gateway** | Add the Mosofin MCP server — `https://mcp.mosofin.com/mcp`. In Claude Code use `claude mcp` or `/mcp`; on claude.ai use connector settings. Authentication is **OAuth** — no credentials are stored locally, and **this skill never asks you for a password, token or account number.** |
